...
contract PrivacyCTV2 is TRCAnonymizerTomoZ, RingCTVerifier, BulletProofVerifier {
    ...
    uint256 public DEPOSIT_FEE = 10**6;//deposit fee = 0.001 TOMO = 10^6 GWEI
    uint256 public FEE = 10**7;//send & withdraw fee = 0.01 TOMO = 10^7 GWEI
    uint8 constant ANONYMIZER_DECIMALS = 9;
    ...
    ITRCAPIBase public token;
    ITRC21 public trc21Token;
    uint8 public token_decimals;
    uint256 public tokenMinFee;
    ...
    constructor (address _tokenAddress, bool isTRC21) public {
        token = ITRCAPIBase(_tokenAddress);
        require(token.decimals() >= ANONYMIZER_DECIMALS, "decimals of the target token must be at least 9");
        token_decimals = token.decimals();
        if (isTRC21) {
            trc21Token = ITRC21(_tokenAddress);
        }
    }
    ...
    */
    function deposit(uint _pubkeyX,
        ...
        require(_value.decimalConvert(token_decimals, ANONYMIZER_DECIMALS) > DEPOSIT_FEE, "deposit amount must be strictly greater than deposit fee");
        //transfer token first
        require(token.transferFrom(msg.sender, address(this), _value), "the sender has not allowed the contract to transfer token");
        transferFee(DEPOSIT_FEE);
        require(Secp256k1.onCurveXY(_pubkeyX, _pubkeyY));
        require(Secp256k1.onCurveXY(_txPubKeyX, _txPubKeyY));

        (uint8 _ybitComitment, uint xCommitment) = Secp256k1.pedersenCommitment(
                                                    _mask, 
                                                    _value.decimalConvert(
                                                        token_decimals, 
                                                        ANONYMIZER_DECIMALS).sub(DEPOSIT_FEE)
                                                    );
    ...
    */
    function withdrawFunds(uint[] memory _inputIDs, //multiple rings
        ...
        //transfer
        transferToken(_recipient, _withdrawalAmount);
    ...
    function transferFee(uint256 fee) internal {
        uint256 realReceivedFee = fee.decimalConvert(ANONYMIZER_DECIMALS, token_decimals);
        if (address(trc21Token) != address(0x00)) {
            require(realReceivedFee > trc21Token.estimateFee(realReceivedFee));
            realReceivedFee = realReceivedFee.sub(trc21Token.estimateFee(realReceivedFee));
        }
        token.transfer(issuer(), realReceivedFee);
        emit TransactionFee(issuer(), realReceivedFee);
    }

    function transferToken(address _to, uint256 _amount) internal {
        uint256 realReceived = _amount;
        if (address(trc21Token) != address(0x00)) {
            require(realReceived > trc21Token.estimateFee(realReceived));
            realReceived = realReceived.sub(trc21Token.estimateFee(realReceived));
        }
        token.transfer(_to, realReceived);
    }
    ...
    function setFees(uint256 _deposit, uint256 _fee) external {
        require(msg.sender == issuer());
        DEPOSIT_FEE = _deposit.decimalConvert(token_decimals, ANONYMIZER_DECIMALS);
        FEE = _fee.decimalConvert(token_decimals, ANONYMIZER_DECIMALS);
    }
